...
static void gen_eob(DisasContext *s, int last_opc);
#else
...
#if defined(INST_BASED_CALL_STACK) || 1
        s->tb->last_opc = 0xFFFE;
#endif
    } else {
                               ...
                               target_ulong pc_start)
{
 ...
 reswitch:
    switch(b) {
    case 0x13f: /* crete custom insturctions*/
        {
#if defined(CRETE_CONFIG) || 1
            uint64_t arg = cpu_ldq_code(env, s->pc);
            tcg_gen_movi_i64(cpu_tmp1_i64, arg);
            ...
            ldq_code(s->pc);
#endif //#if defined(CRETE_CONFIG) || 1
        s->pc+=8;
        }
        break;
    case 0x0f:
    ...
    case 0xd8 ... 0xdf:
        if (s->flags & (HF_EM_MASK | HF_TS_MASK)) {
            ...
            gen_exception(s, EXCP07_PREX, pc_start - s->cs_base);
//            fprintf(stderr, "[floats] EXCP07_PREX generated: s->flags = 0x%p\n",
//            		(void *)s->flags);
            break;
        }
        modrm = cpu_ldub_code(env, s->pc++);
...
#if defined(INST_BASED_CALL_STACK) || 1
            s->tb->last_opc = 0xe8;
#endif //#if defined(INST_BASED_CALL_STACK)
        }
        break;
    case 0x9a: /* lcall im */
        {
...
/* generate intermediate code in gen_opc_buf and gen_opparam_buf for
   ...
   crete:
   @caller_crete: true if being called by crete code
   @crete_end_pc: when non-zero, translate until (not include)
   the guest instruction from the address of crete_end_pc */
static inline void gen_intermediate_code_internal(X86CPU *cpu,
                                                  TranslationBlock *tb,
                                                  bool search_pc,
                                                  bool caller_crete,
                                                  uint64_t crete_end_pc)
{
    CPUState *cs = CPU(cpu);
    ...
    gen_tb_start(tb, caller_crete);
    for(;;) {
#if defined(CRETE_CONFIG) || 1
        // If being called by crete and crete_end_pc is valid
        // Translate until guest eip is "caller_crete"
        if(caller_crete && (crete_end_pc != 0))
        {
            assert(crete_end_pc >= pc_ptr);
            if(crete_end_pc == pc_ptr){
                gen_eob(dc, 0xFFFB);

//                fprintf(stderr, "[CRETE INFO] tb-%p is translated until %p\n",
//                        (void *)(uint64_t)tb->pc, (void *)crete_end_pc);
                break;
            }
        }
#endif
    ...
    if (tb->cflags & CF_LAST_IO)
        gen_io_end();
done_generating:
    gen_tb_end(tb, num_insns, caller_crete);
...
{
    gen_intermediate_code_internal(x86_env_get_cpu(env), tb, false, false, 0);
}

void gen_intermediate_code_pc(CPUX86State *env, TranslationBlock *tb)
{
    gen_intermediate_code_internal(x86_env_get_cpu(env), tb, true, false, 0);
}

void gen_intermediate_code_crete(CPUX86State *env, TranslationBlock *tb, uint64_t crete_end_pc)
{
    gen_intermediate_code_internal(x86_env_get_cpu(env), tb, true, true, crete_end_pc);
}
